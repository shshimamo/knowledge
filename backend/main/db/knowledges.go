...
func (o *Knowledge) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(knowledgeAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(knowledgePrimaryKeyColumns) == 0 {
				return errors.New("db: unable to upsert knowledges, could not build conflict column list")
			}

			conflict = make([]string, len(knowledgePrimaryKeyColumns))
			copy(conflict, knowledgePrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"knowledges\"", updateOnConflict, ret, update, conflict, insert, opts...)
