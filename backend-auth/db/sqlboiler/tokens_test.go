...
var (
	tokenDBTypes = map[string]string{`ID`: `integer`, `AuthUserID`: `bigint`, `Token`: `character varying`, `Active`: `boolean`, `ExpiresAt`: `timestamp without time zone`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_            = bytes.MinRead
)
