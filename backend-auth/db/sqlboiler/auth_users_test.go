...
var (
	authUserDBTypes = map[string]string{`ID`: `integer`, `Email`: `character varying`, `PasswordDigest`: `character varying`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_               = bytes.MinRead
)
